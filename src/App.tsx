...
function App() {
  ...
  useEffect(() => {
    ...
    async function worker() {
      while (!cancelled) {
        ...
        if (current >= apps.length) return;
        const appInfo = apps[current];
        if (iconCache.has(appInfo.iconPath)) continue;
        const dataUrl = await window.ipcRenderer.invoke(
          "apps:icon",
          appInfo.iconPath
        );
        if (cancelled) return;
        iconCache.set(appInfo.iconPath, dataUrl);
        setIconTick((tick) => tick + 1);
      }
    }

    const workers = Array.from(
      { length: ICON_FETCH_CONCURRENCY },
      () => worker()
    );
    Promise.all(workers);
  ...
  async function handleFocus(appInfo: AppInfo) {
    ...
    try {
      ...
      if (!found) {
        showToast(`${appInfo.name} não está em execução.`);
      }
    } finally {
        ...
        {loading ? (
          ...
          <ul className="divide-y divide-border">
            {filteredApps.map((appInfo) => {
              const icon = iconCache.get(appInfo.iconPath);
              return (
                <li
                  key={appInfo.path}
